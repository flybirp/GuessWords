...
public class LearnW2V {

  private Map<String, vec.domain.Neuron> wordMap = new HashMap<>();
  /**
   * 训练多少个特征
   */
  ...
  public LearnW2V(Boolean isCbow, Integer layerSize, Integer window, Double alpha,
                  Double sample) {
    createExpTable();
    if (isCbow != null) {
      this.isCbow = isCbow;
  ...
  private void skipGram(int index, List<WordNeuron> sentence, int b) {
    ...
    for (a = b; a < window * 2 + 1 - b; a++) {
      ...
      for (int i = 0; i < neurons.size(); i++) {
        ...
        // LearnW2V weights hidden -> output
        for (c = 0; c < layerSize; c++) {
          out.syn1[c] += g * we.syn0[c];
        }
      }

      // LearnW2V weights input -> hidden
      for (int j = 0; j < layerSize; j++) {
        we.syn0[j] += neu1e[j];
      }
    }
  ...
  public static void main(String[] args) throws IOException {
    LearnW2V learn = new LearnW2V();
    long start = System.currentTimeMillis();

    String which = "diming";

    learn.learnFile(new File("TrainingData/"+which+"_ready.txt"));
    System.out.println("use time " + (System.currentTimeMillis() - start));
    learn.saveModel(new File("TrainedModels/"+which+"_javaVector"));
